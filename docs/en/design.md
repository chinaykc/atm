...
Markdown task file
  ...
  -> AST
  -> IR
  -> dry-run check plan or execution plan
  -> execution backend
  -> tool adapter
...
- `atm check --plan` renders that IR as text, JSON, or HTML and does not execute tools or bash by default. `check --plan --preview` may evaluate preview-safe lazy providers.
...
- `/db` declarations are parsed by the DSL, resolved by the engine into task-visible DB configs, and exposed by tool adapters as a temporary task tool service. DB files are local JSON `map[string][]string` documents.
...
- **Store package**: `pkg/runtime/store` owns task documents, block leases, locks, managed working files, and atomic writeback.
- **Agent adapter package**: `pkg/integration/agent` owns Codex, Claude, bash, and condition-check adapters.
- **Task document package**: `pkg/workspace/taskdoc` owns file-level format, clean, append, and repair helpers.
- **Public package**: `atm.go` is the importable integration facade for compiling, planning, running, and editing ATM files.
- **Command entrypoint**: `cmd/atm/main.go` only calls the public CLI facade.

The frontend can explain a task without executing it by using the compiler and plan view packages. The engine executes `ir.Task` values and does not reinterpret textual command syntax. Runtime code receives exported IR types such as `ir.Task`, `ir.FlowNode`, `ir.FlatOp`, and `ir.For`.
...
pkg/runtime/store/       task document, block lease, lock, and managed working-file persistence
pkg/integration/agent/   Codex, Claude, bash, and check adapters
pkg/view/plan/           dry-run plan text, JSON, and HTML rendering
pkg/workspace/taskdoc/   file-level format, clean, append, and repair helpers
docs/           user-facing command and design documentation
examples/       editable ATM files
```
...
- `blocks.go` and `markdown_parser.go` own Markdown/plain-text task block discovery.
...
- `syntax` provides shared diagnostic and source-span types used by compiler and IR packages.
...
During a direct run, ATM first creates a managed workspace under `~/.atm/runs/<run-id>/`, copies the source and import files there, hides the original paths behind placeholders, and restores the originals unchanged on normal exit or handled interrupt.

Each task has an independent artifact directory under `~/.atm/runs/<run-id>/tasks/<task-id>/`. Human-readable logs live in that task directory's `logs/` subdirectory; native agent event streams, task-level structured outputs, and `report.md` live beside it. Run-local DB files and `result.md` live under the selected run output directory. The final status document is copied to `~/.atm/runs/<run-id>/result.todo.md`.

Project-persistent `/db persist:project` files live under `.atm/db` in the source ATM file's project directory. Run-local `/db persist:run` files live under the selected run output directory. DB writes use a sidecar lock file plus temp-file rename to serialize local task tool writes.
...
- ATM files are the user-facing API. Existing commands and generated state blocks should remain readable by future versions.
