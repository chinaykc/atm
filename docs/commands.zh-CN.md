...
路径会先用当前任务变量渲染，再相对当前任务工作区或原始 todo 文件所在目录解析。解析后的路径必须留在原始 todo 文件目录内。`/cd` 会影响 Codex/Claude、`/bash`、`/let ... /bash`、自然语言 `until` 检查和本地表达式文件函数；不会改变 ATM 写状态块、输出产物或 DB 文件的位置。
...
在 Markdown 任务文档中，`/context #Heading` 是另一个 task header 命令，用于把远处 section 的普通文档加入当前任务上下文；它不会渲染名为 `context` 的变量。

`/let` 也可以捕获 bash 的 stdout。`/let name /bash ...` 和 `/let name /call ...` 是懒 provider：ATM 在 task header 中记录它们，`atm plan` 和 `atm check` 不会执行；只有变量真正被渲染或被表达式读取时才执行。同一次任务调用内第一次读取后会缓存结果，所以重复写 `{{name}}` 只复用一次结果。需要多次执行定义或 shell 命令时，使用多个绑定，或者直接写 `/call`、`/bash` 命令。

因为 lazy provider 是渲染期依赖，`atm check` 和静态 `atm plan` 会把它们报告为 warning，而不是执行它们。Lazy bash provider 会标记为潜在副作用点；lazy call provider 会标记为 definition 依赖。计划 flow 中分别显示为 `LazyBash(name)` 或 `LazyCall(def -> name)`。
...
声明轻量任务数据库，供 agent 作为记忆或黑板使用。数据库存储 `map[string][]string`，并通过 MCP server 暴露给 Codex 和 Claude。`/db new` 是 scoped 声明块，不会运行 agent：
...
`/skill new` 和 `/mcp new` 是 scoped 声明块。文档根部声明对全文后续任务可见；heading 下的声明只对该 heading 的后续任务和子 heading 可见。同级 heading 不继承，声明也不会被前置提升。`/skill use` 和 `/mcp use` 必须引用当前任务可见的声明；例外是 `/skill use` 可以直接给 path-like 值。
...
Definition 必须包含且只包含一个 `/return`，并且 `/return` 必须是 definition 的最后一个 block。结构化 `/output` 不再作为返回值 fallback。`/return` 写在 `/def` body 之外是错误；普通任务如果只是想提到这个词，应写成普通 Markdown 文本。
...
结构化 `/return` 使用和结构化 `/output` 相同的 MCP output 机制，但 JSON 主要作为调用返回值使用。为了和文本返回区分，结构化返回必须使用明确的 `json`、`yaml`、`yml` 或 `schema` fence；`/return` 后的空 fence 表示普通文本：
...
Import 只加载定义，不会执行被导入文件里的普通任务，也不会导入被导入文件中的 `/db new`、`/skill new`、`/mcp new` 或其他资源声明。路径相对当前 todo 文件。带命名空间的导入通过 `weather.lookup` 调用。Import 使用 Markdown 词法作用域：文档根部的 import 对全文后续任务可见；heading 下的 import 只对该 heading 的后续任务和子 heading 可见。同级 heading 不继承，import 也不会被前置提升。Imported definition 执行时使用调用点的 workdir、DB、skill 和 MCP 视图；调用方需要在导入方文档中显式声明并启用需要的资源。ATM 会检测递归 import 和递归定义调用，包括跨文件 import 形成的环，并在 plan/parse 阶段失败。
...
固定次数循环只绑定小写 `n`。取值从 `0` 开始，所以 `/for 3` 会以 `n = 0`、`n = 1`、`n = 2` 执行；不会再生成大写 `N`。
...
`/if(...)` 和 `/if (...)` 使用本地表达式，和 `until(...)` 对齐。`/if 自然语言条件` 使用所选工具的结构化 MCP check，和自然语言 `until` 对齐：
...
`in expr`、`in(expr)` 和 `in (expr)` 都可以。表达式会在运行期求值。它可以是返回 list 的本地表达式，也可以是返回 list 的带括号 `/call name`，例如 `in(/call plan_shards)`。如果 `/call` 返回带 `plans` 数组的对象，ATM 会展开这个数组。标量项用 `{{plan}}` 渲染；对象项可以用 `{{area.name}}` 这类字段访问。需要整数区间时可以用 Python 风格 `range(stop)`、`range(start, stop)` 或 `range(start, stop, step)`；`step` 不能是 `0`。用 `files()` 和 `dirs()` 枚举当前 task workdir 下一层条目，用 `walkFiles()` 和 `walkDirs()` 递归枚举；它们都支持可选 root，例如 `walkFiles("src")` 或 `walkFiles(outputDir("reports"))`。`.git`、`node_modules`、`vendor`、`dist`、`build` 等生成或依赖目录会被跳过。这是跨版本稳定规则：旧的 `/for file`、`/for dir` 和 `/for path` 控制头无效，文件和目录枚举必须写成表达式形式。如果动态序列为空，ATM 会输出运行时 warning 并跳过循环体。
...
`/go` 不会隐式汇合。文档里如果有后台任务但后面没有匹配的 `/wait`，`atm check` 和 `atm plan` 会给 warning，但仍接受该文件。`atm run` 在没有前台任务后会退出；未匹配的后台 block 可能保持 `running` 状态，并在日志中报告为没有 `/wait` 的后台工作。这适合有意启动监控类后台 agent 的场景；普通 fan-out 工作流建议显式写 `/wait`。
...
`atm check` 和 `atm plan` 会在后台任务没有后续匹配 `/wait` 时给出 warning；工作流依赖后台结果时必须写 `/wait`。

## Plan Dry-Run

使用 `atm plan` 查看命令顺序会如何解释。它只解析 todo 文件并打印文档标题和 Markdown 章节树、全局声明、条件控制块、循环展开摘要、条件分支/skipped 摘要、每个可运行块的源行号和 Markdown scope/title path、默认上下文摘要、每个 task 的执行/跳过决策摘要、任务流程、变量引用/来源摘要、task runtime environment、任务级 DB/skill/MCP 配置、解析后的任务资源视图、异步 fan-out 和 `/wait` 汇合摘要、runner 参数。decision 摘要会标出前台执行、后台 dispatch、纯 `/wait` join、`/wait` 协调任务、条件分支、parent/child 依赖，以及分支 skipped/no-op 原因。runtime 摘要会从 flow 中提取 resume 模式、args、`/cd`、agent 前置 `/bash` 和 lazy provider。资源视图会展开默认 `scope:global` DB 挂载，以及显式 `/db use/access/ignore`、`/skill use`、`/mcp use` 和 `/mcp def use` 设置。默认 `plan` 是静态 dry-run：不运行 bash，不调用所选工具，不执行 lazy provider，也不写入结果块。
...
`atm plan dry-run todo.txt` 和 `atm plan dry-run -file todo.txt` 也可以作为显式别名使用。
...
- `imports`、`dbs`、`skills`、`mcps` 和 `controls` 会在存在时暴露解析后的声明或控制块；`tasks[].db`、`tasks[].skill` 和 `tasks[].mcp` 会在存在时暴露任务级工具配置；`tasks[].resources` 暴露当前 task 实际接收的 DB/skill/MCP/def-MCP 资源视图。
...
结果块由 `<!-- atm:report ... -->` 和 `<!-- /atm:report -->` 包住；ATM 替换时以这个边界为准。中间的 `> [!ATM]` 引用块是给人和 agent 看的可见摘要。`id` 是这个任务/report 的稳定身份；`source` 是写入结果块时根据任务源文本计算的 `sha256`；`rendered` 是变量和 lazy provider 解析后实际发送给 agent 的 prompt hash；`report` 指向 `.atm/reports/` 下的详细 Markdown 报告。它们让 ATM 能在文档编辑后识别“这是哪个任务的报告”，而不是只依赖当前位置。旧结果块可以没有 HTML 边界和这些字段，但当前生成的结果块会写入它们。如果同一文档中出现重复 `id`，`atm check`、`atm plan` 和执行前解析都会报错，要求先修复重复身份再继续。
...
Running 结果块让中断或失败的循环从剩余工作继续，而不是从零开始。`atm` 只会在持有 block lease 时替换尾部生成的 `> [!ATM]` 块；如果任务正文被编辑，lease 会失效并重新扫描。`id`、`source`、`rendered` 和 `report` 会随 running/done/skipped 状态一起写入，用于后续去重和报告关联。任务完成后，ATM 会在原 todo 文件所在目录写入 `.atm/reports/<id>.md` 详细报告；主文档只保留轻量摘要。
...
失败任务会写 `failed` 结果块、`.atm/reports/<id>.md` 详细报告和 `.atm/state.json` 状态。`failed` 是终端生成状态：后续 `atm run` 不会把它当作待执行任务静默重跑；确认原因并修改任务后，用 `atm untag` 移除生成状态再重新执行。
...
原 todo 文件所在目录下还会维护 `.atm/state.json`、`.atm/reports/*.md`、`.atm/logs/*.log` 和短生命周期 `.atm/lock`。`state.json` 是按稳定 task/report id 组织的机器可恢复状态，记录 status、source/rendered prompt hash、plan hash、report 路径、日志路径和运行次数；`.atm/reports/` 保存每个任务的详细 Markdown 报告；`.atm/logs/` 保存每个任务块的人类可读 stdout/stderr 合并日志；`.atm/lock` 用来串行化同一文档的本地写入。`sourcePromptHash` 基于 task 源文本和会进入 prompt 的可见 Markdown 上下文，包括显式 `/context` 引用的章节，但不包括 `/doc` 排除的文本，也不包括 child task 运行结果；`renderedPromptHash` 基于实际发给 agent 的 prompt，包括变量、lazy provider 展开结果和可见 child task report 摘要。它们不受 `-output DIR` 改变，主文档仍是人类可读的轻量协作记录。

如果任务运行期间对应 task block 被删除或改到 lease 失效，ATM 不会把完成结果写回主文档，但会写 `.atm/reports/<id>.md`，在 `.atm/state.json` 里标记 `"orphan": true`，并在命令行输出 orphan report 提示。

`atm check` 会检查主文档 report block、`.atm/state.json` 和 `.atm/reports/*.md` 的明显不一致，并以 warning 报告缺失 detail report、state 中没有主文档对应项的 task id、主文档与 state 的 status/report 路径/source hash/rendered prompt hash 不一致，以及 orphan detail report。

如果多个输入文件共用同一个 `-output DIR`，`atm` 会在 `DIR` 下为每个文件创建一个带编号的子目录，避免结果文档和原生事件流互相覆盖。

### `exec`

执行启动时的 pending prompt block 快照：
...
`exec` 接受和 `run` 相同的工具、输出目录、消息数量、并发参数、状态文件、报告文件和锁机制，但会在启动时冻结 task block 集合。之后通过 `atm append` 或手动编辑追加的新 task block 会保留在文档中，但不会被这次 `exec` 执行；需要再次运行 `atm run` 或 `atm exec` 才会拾取。对启动时已有 task block 的编辑仍会经过 source hash 和 block lease 检查，所以旧快照完成后可能变成 orphan report，而不会把已编辑的新正文错误标记为 done。

`run` 用于持续协作：它会反复重扫活跃 todo 文件，当前 run 仍在运行时追加的任务可能被同一次 run 执行。`exec` 用于发布、CI 或可复现的一次性执行：它把本次运行绑定到命令启动时可见的任务集合。
...
`report` 会读取主文档 report block、`.atm/state.json` 和 `.atm/reports/*.md`，输出 `done`、`running`、`failed`、`skipped` 和 `draft` 数量，并列出失败任务、orphan report 和最近日志路径。`draft` 表示当前文档中仍会被编译为待执行工作的 task block。需要给工具或 CI 消费时使用 `-json`；任务项会包含可用的 `id`、`status`、`report`、`source` 和 `rendered` 字段。
...
不带清理选项时，`clean` 只移除主文档中的生成 report/status block，保留 `.atm/state.json`、`.atm/reports/` 和 `.atm/logs/`。显式选项用于删除审计产物：`--reports` 删除 `.atm/reports/`，`--state` 删除 `.atm/state.json`，`--logs` 删除 `.atm/logs/`，`--all` 同时清理主文档状态块和这些 `.atm` 产物。`clean` 支持位置文件参数，也支持 `-file PATH`。

### `repair-ids`

修复主文档里重复的 ATM report identity：
...
如果用户复制了带生成 report block 的任务，文档可能出现重复 `id`，`atm check` 会失败。`repair-ids` 保留第一个 id，给后续重复项按该 task block 重新生成唯一 id、source hash 和 report 路径。它只修改主文档中的 report identity，不删除 `.atm/state.json` 或 `.atm/reports/`。修复后可以继续用 `atm report` 或 `atm check` 查看剩余审计 warning，再按需要用 `atm clean` 清理陈旧审计产物。
...
- 人类可读任务日志写入源 todo 文件旁的 `.atm/logs/`。
