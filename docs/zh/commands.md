...
路径会先用当前任务变量渲染，再相对当前任务工作区或任务执行根目录解析。对 `atm run` 和 `atm resume` 来说，这个根目录是启动运行时所在的目录，即使 ATM 实际执行的是 `~/.atm/runs/<run-id>/work` 下的托管工作副本。解析后的路径必须留在任务执行根目录内。`/cd` 会影响 Codex/Claude、`/bash`、`/let ... /bash`、自然语言 `until` 检查和本地表达式文件函数；不会改变 ATM 写状态块、输出产物或 DB 文件的位置。
