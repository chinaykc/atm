...
当前语言模型已经是 Markdown 原生模型：标题提供文档、上下文和词法作用域；可执行工作从 `/task`、任务启动 slash 控制命令，或带后续 prompt 的 task header 命令开始。语法权威文档是 [commands.md](commands.md)。
...
- `atm check --plan` 把 IR 渲染成文本、JSON 或 HTML，默认不执行工具或 bash。`check --plan --preview` 可以执行适合预览的 lazy provider。
...
- `/db` 声明由 DSL 解析，engine 解析成当前任务可见的 DB 配置，工具适配器再通过临时任务工具服务 暴露给 agent。DB 文件是本地 JSON `map[string][]string` 文档。
...
- **Store 包**：`pkg/runtime/store` 负责任务文档、块租约、锁、托管工作文件和原子写回。
- **Agent 适配包**：`pkg/integration/agent` 负责 Codex、Claude、bash 和条件检查适配器。
- **Task document 包**：`pkg/workspace/taskdoc` 负责文件级 format、clean、append 和 repair helper。
- **公共包**：`atm.go` 是可 import 的集成门面，负责暴露 compile、plan、run 和 atm 文件维护能力。
- **命令入口**：`cmd/atm/main.go` 只调用公共 CLI 门面。

前端应该能够通过 compiler 和 plan view 包在不执行任务的情况下解释任务。执行器运行 `ir.Task`，运行时代码接收 `ir.Task`、`ir.FlowNode`、`ir.FlatOp`、`ir.For` 等导出的 IR 类型。
...
pkg/runtime/store/       任务文档、块租约、锁和托管工作文件持久化
pkg/integration/agent/   Codex、Claude、bash 和检查适配器
pkg/view/plan/           dry-run plan 文本、JSON 和 HTML 渲染
pkg/workspace/taskdoc/   文件级 format、clean、append 和 repair helper
docs/           面向用户的命令和设计文档
examples/       可直接编辑的 atm 文件
```
...
- `blocks.go` 和 `markdown_parser.go` 负责 Markdown/纯文本 task block 发现。
...
- `syntax` 提供 compiler 和 IR 包共享的 diagnostic 与 source span 类型。
...
直接运行期间，ATM 会先在 `~/.atm/runs/<run-id>/` 创建托管运行目录，把源文件和 import 文件复制进去，用占位文件隐藏原路径，并在正常退出或可处理的中断发生时恢复原文件不变。

每个任务在 `~/.atm/runs/<run-id>/tasks/<task-id>/` 下有独立产物目录。人类可读日志位于该任务目录的 `logs/` 子目录；agent 原生事件流、任务级结构化输出和 `report.md` 位于同一任务目录。run-local DB 文件和 `result.md` 位于本次选择的 run output 目录。最终带状态文档会复制到 `~/.atm/runs/<run-id>/result.todo.md`。

`/db persist:project` 文件位于源 atm 文件项目目录下的 `.atm/db`。`/db persist:run` 文件位于本次 run 的 output 目录。DB 写入使用旁路 lock 文件加临时文件 rename，串行化本地 任务工具写操作。
...
- atm 文件是用户可见 API。已有命令和生成状态块应能被未来版本读取。
- 新增适配器不应改变任务块语法。
