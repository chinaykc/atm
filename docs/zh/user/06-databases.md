...
`/db` 给任务声明本地 JSON 数据库，并通过 任务工具暴露给 Codex/Claude。它适合保存跨任务共享的事实、并行分支的发现、发布决策、待确认问题和审计记录。
...
声明行后面的正文是 usage description。ATM 会把它放进 任务工具返回结果里，让 agent 知道这个数据库应该怎么用。
...
自然语言 `until` 和 `/if` 检查使用只读 DB 任务工具服务，即使任务本身有写权限，检查 agent 也只能 list/get/scan。
