...
未选中的块会被写成 `> [!ATM] status: skipped`，所以后续扫描会跳过它。`/if` 是任务块级控制流，不是 prompt 内的模板语法。

`/if(...)` 使用本地表达式；`/if 自然语言条件` 会走 agent 的 结构化检查：
...
如果 `/wait` 后面紧跟 prompt，ATM 会先等待匹配的后台任务完成，再用少量 wait 结果上下文启动这个 prompt：
...
这种写法会让 prompt 带上等待范围，以及匹配后台任务的 id、block、pool、最终状态、日志路径、错误和可见 report。
