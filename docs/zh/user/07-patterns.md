...
/go reviewer
审查 docs/zh/commands.md，确认命令说明准确。
...
```sh
atm run todo.txt -output .atm/live-run
```

另一个终端追加：

```sh
printf '/task\n审查刚刚生成的 result.todo.md，提取风险。\n' | atm append todo.txt
```

当前 `run` 使用托管工作副本执行。`append todo.txt` 会先解析当前 active 文件；如果 `todo.txt` 对应的 run 仍在执行，追加内容会写入本次运行的 active 工作文件，并可被 live/rescan 拾取。也可以直接编辑启动时 CLI 打印的 working file。

边界很重要：

- 如果当前 `atm run` 仍在执行，`atm append todo.txt` 会影响当前 run。
- 如果当前 run 已经退出，`append` 会写入源文件，留给下一次 `atm run todo.txt`。
- ATM 当前不是常驻文件 watcher。`append` 只在命令执行时追加一次，不会持续监听目录。
...
/resume release_coordinator /for 3 until 完整验证通过
/args --yolo
...
源文档会恢复为执行前内容；带状态的结果文档、详细日志、结构化事件和最终快照进入托管 run 目录。典型审计结构是：
...
~/.atm/runs/<run-id>/
  ...
  outputs/
  tasks/
    release-notes-final-<hash>/
      report.md
      task-001-run-001-codex.jsonl
      logs/
        task-001-<timestamp>.log
```

状态块由 ATM 拥有，可以整体替换；删除结果文档里的状态块等价于请求重新评估该 task block。`atm check` 会检查结果文档状态块、`work/.atm/state.json` 和任务详细报告的基本一致性，缺失的详细报告或孤儿报告会作为 warning 输出。每个任务的 `report.md` 由 ATM 自动生成，包含状态、source/rendered prompt hash、plan hash、运行次数、输出目录和最近 assistant 消息。
...
| 多项目复用流程 | 工作流库 import |
| 运行中追加后续工作 | `atm append`；run 未退出时影响当前 active 工作文件，退出后影响下次 run |
| 降低并发写冲突 | 只读并行，写入串行 |
