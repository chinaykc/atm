...
func (p *agentEventParser) finish() []agentDisplayEvent {
	...
	message := dsl.OutputMessage{Tool: "claude", Role: "assistant", Text: p.claudeFallback}
...
func (p *agentEventParser) consumeCodex(event map[string]any) []agentDisplayEvent {
	...
	if eventType == "item.completed" && itemType == "agent_message" {
		...
		message := dsl.OutputMessage{Tool: "codex", Role: "assistant", Text: text}
		p.messages = append(p.messages, message)
...
func (p *agentEventParser) consumeClaude(event map[string]any) []agentDisplayEvent {
	...
	if len(textParts) > 0 {
		p.messages = append(p.messages, dsl.OutputMessage{Tool: "claude", Role: role, Text: strings.Join(textParts, "\n")})
	}
	return events
}

func parseCodexMessages(output string) []dsl.OutputMessage {
...
func parseClaudeMessages(output string) []dsl.OutputMessage {
