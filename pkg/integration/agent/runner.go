...
type Runner interface {
	Name() string
	Execute(ctx context.Context, todoPath, prompt string, opts ir.RunOptions, stdout, stderr io.Writer) (ExecuteResult, error)
	Check(ctx context.Context, todoPath, prompt, condition string, opts ir.RunOptions, stdout, stderr io.Writer) (bool, error)
}
...
func codexOutputMCPArgs(spec *ir.OutputSpec, resultFile, schemaFile string) []string {
	server := outputMCPServer(resultFile, schemaFile, spec.SchemaFormat)
	args := codexMCPServerConfigArgs(outputMCPName, server)
	args = append(args, "-c", "mcp_servers."+outputMCPName+".tools."+mcp.OutputToolName+".approval_mode=\"approve\"")
	return args
}

func codexMCPServerConfigArgs(name string, server mcpServerSpec) []string {
	if server.url != "" {
		return []string{"-c", "mcp_servers." + name + ".url=" + tomlString(server.url)}
	}
	return []string{
...
func codexCheckArgs(opts ir.RunOptions, resultFile string, dbConfigFiles ...string) []string {
	...
	configArgs = append(codexMCPServerConfigArgs(mcpServerName, server), configArgs...)
	if dbConfigFile != "" {
...
func codexDBMCPArgs(configFile string, readonly bool) []string {
	...
	args := codexMCPServerConfigArgs(dbMCPName, server)
...
func codexDefsMCPArgs(configFile string, spec *ir.DefMCPRuntime) []string {
	server := defsMCPServer(configFile, spec)
	args := codexMCPServerConfigArgs(defsMCPName, server)
...
func defMCPToolNames(spec *ir.DefMCPRuntime) []string {
	if spec == nil {
		return nil
	}
	...
	for _, ref := range spec.Defs {
		tool := ir.DefMCPToolName(ref.Name)
...
func codexExternalMCPArgs(mcps []ir.MCPRuntime) []string {
	var args []string
	for _, item := range mcps {
		...
		args = append(args, codexMCPServerConfigArgs(item.Name, server)...)
		if server.url == "" {
...
func (r claudeRunner) Execute(ctx context.Context, todoPath, prompt string, opts ir.RunOptions, stdout, stderr io.Writer) (ExecuteResult, error) {
...
func claudeCheckArgs(prompt string, opts ir.RunOptions, resultFile string, dbConfigFiles ...string) []string {
	dbConfigFile := ""
	if len(dbConfigFiles) > 0 {
		dbConfigFile = dbConfigFiles[0]
	}
...
func runCheckCommand(ctx context.Context, path string, argsForPrompt func(string, string, string) []string, todoPath string, builder checkPromptBuilder, prompt, condition string, opts ir.RunOptions, useStdin, preferMCP bool, streamTool string, stdout, stderr io.Writer) (bool, error) {
	resultFile := ""
	cleanup := func() {}
	if preferMCP {
		var err error
...
func prepareOutputMCP(spec *ir.OutputSpec) (resultFile, schemaFile string, cleanup func(), err error) {
	cleanup = func() {}
...
func prepareDBMCP(dbs []ir.DBRuntime) (configFile string, cleanup func(), err error) {
	cleanup = func() {}
	if len(dbs) == 0 {
		return "", cleanup, nil
	}
...
func prepareDefMCP(spec *ir.DefMCPRuntime) (configFile string, cleanup func(), err error) {
	cleanup = func() {}
...
func prepareSkills(adapter string, opts ir.RunOptions) (func(), error) {
...
func checkMCPServer(resultFile string) mcpServerSpec {
	if networkMCPEnabled() {
		if endpoint, err := mcp.RegisterNetworkCheck(resultFile); err == nil {
			return mcpServerSpec{url: endpoint.URL}
		}
	}
...
func networkMCPEnabled() bool {
	return os.Getenv("ATM_MCP_TRANSPORT") != "stdio"
}

func outputMCPServer(resultFile, schemaFile, schemaFormat string) mcpServerSpec {
	if networkMCPEnabled() {
		if schema, err := os.ReadFile(schemaFile); err == nil {
			if endpoint, err := mcp.RegisterNetworkOutput(resultFile, string(schema), schemaFormat); err == nil {
				return mcpServerSpec{url: endpoint.URL}
			}
		}
	}
...
func dbMCPServer(configFile string, readonly bool) mcpServerSpec {
	if networkMCPEnabled() {
		if endpoint, err := mcp.RegisterNetworkDBConfig(configFile, readonly); err == nil {
			return mcpServerSpec{url: endpoint.URL}
		}
	}
	exe, err := os.Executable()
...
func defsMCPServer(configFile string, spec *ir.DefMCPRuntime) mcpServerSpec {
	if spec != nil && spec.URL != "" {
		return mcpServerSpec{url: spec.URL}
	}
	exe, err := os.Executable()
	if err != nil || exe == "" {
		exe = "atm"
...
func checkMCPConfigJSON(resultFile, dbConfigFile string) string {
	...
	servers := map[string]any{
		mcpServerName: claudeMCPServerConfig(check),
	}
	if check.url == "" {
	...
	if dbConfigFile != "" {
		...
		servers[dbMCPName] = claudeMCPServerConfig(db)
	}
...
func executeMCPConfigJSON(spec *ir.OutputSpec, resultFile, schemaFile, dbConfigFile, defConfigFile string, mcps []ir.MCPRuntime, defMCP *ir.DefMCPRuntime, readonlyDB bool) string {
	servers := map[string]any{}
	if spec != nil && spec.IsStructured() {
		output := outputMCPServer(resultFile, schemaFile, spec.SchemaFormat)
		servers[outputMCPName] = claudeMCPServerConfig(output)
	}
	if dbConfigFile != "" {
	...
	if defConfigFile != "" {
		defs := defsMCPServer(defConfigFile, defMCP)
		servers[defsMCPName] = claudeMCPServerConfig(defs)
	}
	for _, item := range mcps {
...
func claudeMCPServerConfig(server mcpServerSpec) map[string]any {
	if server.url != "" {
...
func parseRuntimeMCP(item ir.MCPRuntime) (mcpServerSpec, error) {
...
func parseMCPServerConfig(name string, data json.RawMessage) (mcpServerSpec, error) {
	...
	if cfg.URL != "" {
		return mcpServerSpec{url: cfg.URL, env: cfg.Env}, nil
	}
	if cfg.Command == "" {
		return mcpServerSpec{}, fmt.Errorf("mcp %q command is required", name)
	}
	return mcpServerSpec{command: cfg.Command, args: slices.Clone(cfg.Args), env: cfg.Env}, nil
}

func runtimeMCPConfigMap(item ir.MCPRuntime) (map[string]any, error) {
	server, err := parseRuntimeMCP(item)
	if err != nil {
		return nil, err
	}
...
func validateRuntimeMCPs(items []ir.MCPRuntime) error {
