...
你会看到 ATM 最多启动 3 次 agent 执行同一个修复任务。每次执行后，ATM 会让 agent 用结构化检查报告“测试全部通过”是否成立；如果成立就提前停止，否则继续下一轮。生成的结果文档会记录每轮执行状态、保留最近的 assistant 消息，并把详细日志放进本次 run 目录。
...
/for dir in dirs()
/go
审计 {{dir}} 目录，报告写入 ./report/{{dir}}.md。

/wait
汇总 ./report/*.md 到 report.md，并列出仍需人工确认的问题。
```
...
你会看到 ATM 在运行期用 `dirs()` 枚举当前目录下的一层子目录，为每个目录启动一个后台 agent 分支；`/wait` 会先汇合这些分支，再启动汇总 prompt。汇总 prompt 会附带少量 wait 结果上下文，包括每个后台分支的 async id、block、pool、最终状态、日志路径、错误和可见 report，便于生成最终 `report.md`。

ATM 定义了一套语法规则，适用于介于 `一次性 prompt` 和 `完整工作流系统` 之间的场景：当一个项目需要让 agent 做多件事，例如运行检查、审查文档、验证示例、修复问题时，聊天窗口很快会变得难以审计、难以复用，也难以看清任务之间的关系。ATM 让你把这些工作写进一个可读的 `todo.md`：项目背景仍然是普通 Markdown，少量斜杠命令负责描述任务如何执行。
